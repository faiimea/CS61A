...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            # print('DEBUG',self.can_contain(place.ant))
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        tmp_dist = 0
        while (not tmp_place is beehive):
            if not tmp_place.bees==[] and self.min_range <= tmp_dist <= self.max_range:
                return bee_selector(tmp_place.bees)
            tmp_place=tmp_place.entrance
            tmp_dist+=1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in lst:
            ...
            # print("DEBUG",i.place)
        self.place.bees=[i for i in lst if i.health>0]
        lst=self.place.bees[:]
    ...
    def action(self, gamestate):
        if self.chewing ==0:
            if not self.place.bees == []:
                tmp_bee=bee_selector(self.place.bees)
                tmp_bee.reduce_health(tmp_bee.health)
