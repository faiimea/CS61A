def couple(s, t):
    ...
    assert len(s) == len(t)
    "*** YOUR CODE HERE ***"
    ans=[[s.pop(),t.pop()] for i in range(len(s))]
    ans.reverse()
    return ans
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp_city=make_city('faii',lat,lon)
...
def berry_finder(t):
    ...
    if label(t)=='berry':
        ...
        if_berry=False
        for i in branches(t):
            if_berry=if_berry or berry_finder(i)
        return if_berry
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        tmp_label=label(t)
        tmp_list=[tree(i) for i in leaves]
        return tree(tmp_label,tmp_list)
        # t[1][0:0]=list(leaves)
        return t
    else:
        tmp_list=[]
        for i in branches(t):
            tmp_list.append(sprout_leaves(i,leaves))
        return tree(label(t),tmp_list)
